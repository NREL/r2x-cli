...
pub fn parse_plugin_json(
    ...
    if let Some(plugins_array) = package.get("plugins").and_then(|p| p.as_array()) {
        for plugin_obj in plugins_array {
            ...
            {
                let version_strategy = plugin_obj
                    .get("version_strategy")
                    .and_then(|v| serde_json::from_value(v.clone()).ok());

                let version_reader = plugin_obj
                    .get("version_reader")
                    .and_then(|v| serde_json::from_value(v.clone()).ok());

                let upgrade_steps = plugin_obj
                    .get("upgrade_steps")
                    .and_then(|u| serde_json::from_value(u.clone()).ok());

                if version_strategy.is_some() || version_reader.is_some() || upgrade_steps.is_some() {
