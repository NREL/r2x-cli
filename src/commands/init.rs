...
pub fn handle_init(filename: Option<String>, _opts: GlobalOpts) {
    ...
    match fs::write(&target_filename, PIPELINE_TEMPLATE) {
        Ok(_) => {
            ...
            println!();
            println!("Next steps:");
            println!("  1. Edit {} with your pipeline configuration", target_filename.bold());
            println!("  2. Install plugins: r2x install <package>");
            println!("  3. List available pipelines: r2x run {} --list", target_filename);
            println!("  4. Run a pipeline: r2x run {} <pipeline-name>", target_filename);
            println!("  5. Preview pipeline: r2x run {} <pipeline-name> --dry-run", target_filename);
        }
        Err(e) => {
