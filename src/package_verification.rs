...
pub fn verify_plugin_packages(
    ...
    let plugin = match manifest.get_plugin(plugin_key) {
        Some(p) => p,
        None => {
    ...
    // Get package name from plugin metadata
    let package_name = match &plugin.package_name {
        Some(name) => name,
        None => {
            return Err(VerificationError::VerificationFailed(format!(
                "Plugin '{}' has no package_name in manifest",
                plugin_key
            )))
        }
    };
    ...
    let missing_packages = check_packages_installed(&venv_path, &[package_name.as_str()])?;
...
pub fn verify_all_packages(
    ...
    for (_key, plugin) in manifest.list_plugins() {
        if let Some(package_name) = &plugin.package_name {
            all_packages.insert(package_name.as_str());
        }
    }
